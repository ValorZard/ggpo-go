// Package webrtcconn establishes WebRTC data channel connections between GGPO
// peers using the lobby-based signaling server from the signaling package.
//
// The host creates a lobby and accepts joining players; each side receives a
// detached, message-oriented data channel (an io.ReadWriteCloser) which can be
// registered on a transport.DataChannel and used as a GGPO connection:
//
//	dialer := webrtcconn.NewDialer("http://127.0.0.1:3000")
//	lobby, _ := dialer.HostLobby(ctx)          // share lobby.ID out of band
//	channel, playerID, _ := lobby.Accept(ctx)  // one call per remote player
//
//	peer, dcTransport := ggpo.NewDataChannelPeer(session, numPlayers, inputSize)
//	dcTransport.AddPeerChannel("player", playerID, channel)
//
// Data channels are created unordered with no retransmissions, matching the
// UDP semantics GGPO's protocol is designed around (it does its own
...
type Dialer struct {
	...
	SignalingURL string
	// ICEServers lists the STUN/TURN servers used for connectivity. Empty
	// means host candidates only, which works on a LAN or localhost. For
	// connections across the internet supply at least a STUN server, e.g.
	...
	// HTTPClient is used to talk to the signaling server. Defaults to
	// http.DefaultClient.
...
func NewDialer(signalingURL string) *Dialer {
	// The detached data channel API is required so channels can be used as
	// io.ReadWriteClosers by transport.DataChannel.
...
func (d *Dialer) HostLobby(ctx context.Context) (*Lobby, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("webrtcconn: hosting lobby: %w", err)
	}
...
func (d *Dialer) Join(ctx context.Context, lobbyID string) (io.ReadWriteCloser, int, error) {
	...
	channel, err := d.completeConnection(ctx, pc, lobbyID, pData.ID, channelChan, false)
	if err != nil {
		pc.Close()
		return nil, 0, err
	}
	return channel, pData.ID, nil
}
